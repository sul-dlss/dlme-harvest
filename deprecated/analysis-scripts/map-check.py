...
def get_missing_dates(records):
    for record in records:
        if 'cho_date' in record:
            if 'cho_date_range_norm' in record:
                pass
            else:
                print(f"Record {record['id']} in {record['dlme_source_file']} has unnormalized dates. The date value was: {record['cho_date']}")
...
FUNCTION_MAP = {"build_controlled_vocabulary": build_controlled_vocabulary,
                ...
                "inspect": inspect,
                "get_missing_dates": get_missing_dates,
